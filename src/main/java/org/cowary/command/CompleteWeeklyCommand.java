...
public class CompleteWeeklyCommand implements Callable<Integer> {
    ...
    public Integer call() throws Exception {
        try {
            ...
            List<WeeklyWithCompletionStatus> weeklyList = completedWeeklyService.getIncompleteTasks();
            List<WeeklyWithCompletionStatus> incompleteTasks = weeklyList.stream()
                    .filter(w -> Boolean.FALSE.equals(w.getCompletedToday()))
                    .toList();
            List<WeeklyWithCompletionStatus> completeWeeklyToday = weeklyList.stream()
                    .filter(w -> Boolean.TRUE.equals(w.getCompletedToday()))
                    .toList();
            ...
            System.out.println("\nВыполненные на сегодня задачи");
            for (int i = 0; i < completeWeeklyToday.size(); i++) {
                WeeklyWithCompletionStatus task = completeWeeklyToday.get(i);
            ...
            System.out.println("Возможные к выполнению");
